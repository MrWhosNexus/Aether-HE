...
const makeBoardContextValue = ({ board, roster, selectBoard, refreshBoards, switching }) => {
  ...
  return {
    ...
    isSupported: (name) => (caps ? caps[name] === true : true),   // fail-open on unknown
    selectBoard: selectBoard || (() => Promise.resolve({ ok: false, error: "no bridge" })),
...
const UNKNOWN_BOARD = makeBoardContextValue({ board: null, roster: [] });

const useBoard = () => React.useContext(BoardContext) || UNKNOWN_BOARD;

window.AetherBoard = { BoardContext, useBoard, capState, makeBoardContextValue, UNKNOWN_BOARD };
})();
